...
RESOURCE = os.environ.get(
    ...
    os.environ.get(
        ...
        "https://endurance-pulse-1097189612432.us-east4.run.app",
    ),
)
...
def _auth_headers() -> dict[str, str]:
    if RESOURCE.startswith("https://"):
        try:
            target_aud = RESOURCE.split("/a2a/")[0] if "/a2a/" in RESOURCE else RESOURCE
            req = google.auth.transport.requests.Request()
            token = google.oauth2.id_token.fetch_id_token(req, target_aud)
            return {
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        async for event in a2a_client.send_message(msg):
            ...
            elif getattr(update, "status", None) and getattr(update.status, "message", None):
                msg_obj = update.status.message
                role = getattr(msg_obj, "role", None)
                meta = getattr(msg_obj, "metadata", None) or {}
                if role in (Role.agent, "agent", "Role.agent") and getattr(msg_obj, "parts", None):
                    if not meta.get("adk_error_code"):
                        parts.extend(_extract_parts(msg_obj.parts))

        # Non-streaming / history fallback: pull parts from final task history or artifacts.
        if not parts and last_task is not None:
            ...
            if not parts and getattr(last_task, "history", None):
                for m in last_task.history:
                    ...
                    if role in (Role.agent, "agent", "Role.agent") and getattr(m, "parts", None):
                        if not meta.get("adk_error_code"):
    ...
    for p in parts:
        if (
            p.get("kind") == "text"
            and deduped
            and deduped[-1].get("kind") == "text"
            and deduped[-1].get("text") == p.get("text")
        ):
            continue
        deduped.append(p)
